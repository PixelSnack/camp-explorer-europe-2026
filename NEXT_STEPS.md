...
**Last Updated:** September 5, 2026 (~01:00, exit protocol after the 3 to 5 Sept rollover sessions)
...
## 🔴 **START HERE — SESSION PICKUP (written 5 September 2026, ~01:00)**

### State on exit
Everything is LIVE at www.europeansummercamps.com and verified by the owner on desktop and phone (last push 5 Sept ~00:30). Only this exit-protocol docs commit may still be unpushed; check `git rev-list --count origin/main..HEAD` and ask the owner to push if it is not zero. Tree clean, no dev or preview server running.

What shipped 3 to 5 Sept (full record with every reviewer finding and its disposition: `docs/reports/WAVE1_ROLLOVER_PLAN_2026-09-03.md`):
- Season rollover Wave 1: title and descriptions carry 2027 (rolls every September), every other surface year-agnostic, verified-only booking badges (blue on IDs 1 and 10, green on Filmkollo 37, none elsewhere), visible FAQ mirrored into the JSON-LD, season constants in `src/data/season.js`, validators that fail the build on stale static claims.
- Mobile-first fixes after a Fable review adjudicated on rendered measurements: marquee full travel, reduced-motion wrap, card overlay row that wraps at 320px, long date strings wrap.
- Documentation audit: stale claims corrected across the control docs, README and reference snapshots; five superseded files archived; agent prompts carry a no-fabrication clause (local only, `.claude/` is gitignored).
- Footer country list complete (Romania, Slovenia added). FAQ: 12 questions, accurate against the data, verification work stated, Central and Eastern Europe described for substance, day-camp audience explained, "Our directory".

### Owner items
1. Push this docs commit if `ahead` is not zero.
2. Search Console: Request indexing on the root URL and resubmit sitemap.xml if not done on 4 Sept (both tabs were opened in Chrome; the inspection deep link redirects to the overview, so use the top inspection bar).
3. PayPal tax information was due 5 Sept.
4. Decisions queued below: winter camps (go/no-go and menu name), the alignment line, new-camp priorities.

### Claude, next session, in this order
1. Startup protocol (CLAUDE.md Step 0: bridge inbox; Playground still owes the GSC MCP TLS fix, note `2026-09-04-to-playground-gsc-mcp-tls-broken.md`).
2. **Weekly GSC watch** (from the week of 7 Sept, six weeks): head-term positions and CTR, "2027" query impressions, the title link as displayed. Pull with the standalone script pattern (`scratchpad` is session-bound; the recipe is in `docs/reports/GSC_PULL_2026-09-03.md`: service account + truststore + HTTPLIB2_CA_CERTS, never disable validation). Rollback for the title if the title link misbehaves: `git revert 3314abe`.
3. **ILC payment**: search Gmail for "2026-001" and the five ESC addresses. If unpaid on 10 Sept, draft (never send) a friendly reminder from partnerships@. On payment: activate within 3 business days as camp ID 70 (`featured: true`, `videoUrl` youtu.be/-HKzc8K6Zt0 without the ?si tracker, Hotel Au Vieux Moulin named on the card, CHF international prices, priceRange luxury); entities, prices and the hotel caveat are in `docs/reports/INBOUND_CAMP_VERIFICATION_2026-08-17.md`; log the payment in the cross-brand turnover register.
4. **Wave 2 per-camp 2027 dates** as operators publish (September to December): update `dates:` (under about 40 characters, provenance comment) and set `bookingStatus: "2027 dates published"`; top-clicked first: EUROCAM Bohemia, Altitude Camps, Enforex Barcelona, Quinta da Broeira, Evasoleil, Kalkalpen, Camp California Croatia, Explorer International (Les Elfes, Adventure Camp Bavaria and Filmkollo are done). Warsaw Montessori (ID 24): bookingUrl is a 2025 page and the operator now advertises a children-only camp; re-verify with the operator before touching it, never remove without owner consent. Jagiellonian (ID 28) URL points at an online course (HEALTH_CHECK S8): re-verify. Carried over: IDs 31, 41, 64 ages, 65 rename; Funside 2027 dates early 2027; overdue quarterly price review; review-aggregation Phase 2.
5. **Winter camps decision prep** (owner request 5 Sept): see the sketch below; bring the owner a one-page proposal, do not build before the go.
6. **New camp research** (owner request 5 Sept): ice hockey camps and local camps competitors do not list; see the brief below. Read-only agents, max 5 camps per brief, the no-fabrication clause is in every prompt; verify every number yourself before it reaches camps.js.
7. **Alignment line** (deferred by the owner, 5 Sept): the phrase "Information compiled from public sources" sits in the footer description, the copyright line, the Guide "Safety & Standards" subtitle and the Organization schema description. When the owner picks it up it must stay a SHORT single line (no long text) in all four places; candidate: "Information verified against operator sources and direct enquiries." Do not change without the owner's word (footer and schema carry SEO weight).
8. Still open (unchanged): EmailJS cosmetic pass; CSP report-only to enforce (ask first, console-check 2-3 views); owner dashboard items (Vercel plan, Cloudflare proxy + SPF/DKIM/DMARC, GitHub 2FA + secret scanning, GA Signals, EmailJS "Do not save private data"); camp additions on replies (BELT, CBS, Samiad, LayosCamp); deferred code items (HEALTH_CHECK section 6: Q12 camp-name h3, Q13 to Q19; header nav labels wrap at ~800px; safe-area offset on the fixed FAB and scroll button; 12px stat-description contrast); Les Elfes demo formalization (offer EUR 199 intro or revert to editorial Basic); old EUR 99 quotes decided by the owner case by case if accepted.

### Winter camps: implementation sketch (for the owner's decision)
Context: the 17 Aug gap map recommended winter as a separately labelled section, not an eighth category, and only after the rollover; the rollover is done, so this is unblocked. The brand "Camp Explorer Europe" carries winter naturally; the domain stays the summer ranking asset and is not touched.
- **Navigation**: a separate menu item "Winter Camps" on a new hash route `#winter` (add `winter` to the section whitelist in the hash handler in App.jsx, the one fixed on 17 Aug). Home and Discover stay summer-only so the 65 / 100+ / 24 claims stay true; the winter view shows its own count.
- **Data**: add `season: "winter"` to winter camps (absent = summer); the validator enforces the enum and excludes winter from `allCamps`-derived summer counts (or export `summerCamps` and `winterCamps` from camps.js and switch the counts to `summerCamps`). Same card component, same badge rules; the booking-timeline copy already holds for winter (owner rule 17 Aug). Winter dates run December to March, so the "2027 dates published" badge label still fits.
- **SEO**: title, H1 and descriptions stay summer (do not dilute the ranking asset). The winter view gets an H2, an intro paragraph, one FAQ entry ("Are there winter camps?") and an ItemList entry in the JSON-LD. A real `/winter` URL waits for Phase 2 (React Router + SSG); until then the SPA has one URL and the sitemap stays single-entry. Run the SEO agent and a SOL pass before shipping (touches routing and counts).
- **Content to start with** (verified 17 Aug, all five criteria pass): Stadium Sports Camp (Sweden, residential ice hockey) and Les Elfes Winter (Verbier, same operator as ID 1). Possible: Hockey Talent School Třinec (CZ), UCPA colos ski (FR), La Garenne winter, Leksand. Rejected candidates and reasons: CAMP_EXPANSION_ROADMAP.md Batch 3.
- **Decisions for the owner**: go/no-go; menu label; whether winter camps count in the headline numbers (recommendation: no, state a separate winter count); launch timing (winter bookings run September to November, so a November launch still catches the season).
- **Effort**: one evening for the plumbing plus two to three verified camps; a second evening for content and reviews.

### New camps: research brief (owner request 5 Sept)
- **Ice hockey**: residential hockey schools, not club-only training. Leads: Stadium Sports Camp (SE), Hockey Talent School Třinec (CZ), Leksand (SE), Finnish and Swiss hockey schools; Vierumäki Finnhockey (ID 65) is already listed. Check ages, per-child price, residential status, dates; hockey camps often sell "camp + accommodation" separately, so confirm what the price includes.
- **Local camps competitors do not list**: municipal, association and non-profit camps found by searching in the national language (Ferienlager, colonie de vacances, campo estivo, sommerleir, sommarläger, obóz letni, letní tábor, zomerkamp). Existing examples of the type: Luontoliitto (FI), LINEŠA (LT), AERAT (IT), Foreningen Norden (DK). Gaps from the roadmap: Luxembourg (0 camps), France family or sports, Germany family, Belgium non-language; Netherlands has traffic and only 2 camps.
- **Rules**: camp-content-researcher, max 5 camps per brief, native-language search, all five criteria, per-child pricing, tested URLs, day camps only with the label; every number re-verified by the lead before it enters camps.js; next camp ID is 70 (reserved for ILC on payment, so new camps start at 71 if ILC pays first).

**Standing rules:** visual verification on production after significant changes (Claude in Chrome; the maximized window ignores resize, so phone width runs through the headless DevTools script pattern in LESSONS_LEARNED); testimonials in Resources stay; truthful but not saints, never undersell the verification work (owner, 4 Sept); drafts only, never message third parties (forms and mail to the owner are fine); agents read-only, briefed to answer under 4,500 characters and to say NOT FOUND; scalpel not axe on a ranking site; commit after every step; em-dash ban in outward text; booking copy must hold for winter camps; never conclude a message "was never sent" without a full get_thread read; no badge is the honest default, a badge needs a verified status; never find-and-replace camps.js; FAQ edits go through faq.js and the JSON-LD regeneration or the build fails.
