...
Draft at `docs/drafts/2026-09-06-wave2-data-request-and-base-fee-DRAFT.md`: Wave 2 data requests carry the base fee (EUR 79 one payment, the policy's Basic onboarding figure, applied to editorial listings from the 2027 cycle) and the Premium option (EUR 299, EUR 199 first year); camps that do not respond or pay stay listed for now. Owner confirms the fee figure and unit, then the policy file is updated in the same commit as the first tailored draft. Every message is a Gmail draft the owner sends; Claude never sends.
