...
**State on exit 4 Sept ~10:00:** Season rollover Wave 1 is IMPLEMENTED and COMMITTED, NOT PUSHED. Commits on main since 7058a70, awaiting the owner's push in GitHub Desktop: 244bdc2 (docs, invoice + GSC pull), 9c7099c (plan), db5f003 (data), bb7be98 (FAQ module), a12af02 (App.jsx), cfc7b32 (evergreen metadata, rollback target), 3314abe (title 2027), f2a9af7 (control docs), f9cfb67 (review fixes), 1ac393d and 7b7ea92 (docs), d55e2e7 (code-review closing batch + Filmkollo 2027), da811e9 (closing docs), 78a6feb (README price floor, FAQ 48px, guidelines vocabulary). Fourteen commits in total. Reviews: Fable SEO GO WITH CHANGES, SOL SHIP WITH FIXES, Fable security SHIP, Fable code SHIP WITH FIXES; every accepted item landed. Decision record with the reviewer adjudications: `docs/reports/WAVE1_ROLLOVER_PLAN_2026-09-03.md`. Session memory: `esc-wave1-night-2026-09-03`.
...
5. If the extension cannot reach a page, the CDP fallback is `scratchpad/cdp_verify.mjs` (pattern recorded in LESSONS_LEARNED).
6. Bridge: Playground owes a reply on `2026-09-04-to-playground-gsc-mcp-tls-broken.md` (GSC MCP transport). Until fixed, pull GSC with the standalone script pattern (service account + truststore + HTTPLIB2_CA_CERTS).
