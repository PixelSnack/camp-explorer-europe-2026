...
**Owner decisions read back 6 Sept 21:45:** 14 Accept, 17 Investigate, 2 Reject, with notes; the full table and the next action per camp are in `docs/reports/WINTER_DECISIONS_2026-09-06.md`. Owner correction: skiing is not a criterion; winter camps of every kind qualify (criteria addendum amended). Next session starts with the footprint checks and re-reads of the 14 accepted camps, then one Gmail draft per operator question.
