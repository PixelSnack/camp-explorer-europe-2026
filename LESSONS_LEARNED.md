...
- **Agents idle without reports are a broken channel, not slow agents.** Seven Opus research agents showed idle in ListAgents with no report and ignored a ping. Suspect the user-level Node-based WebFetch hook with no timeout (shared-machine rule: pure bash hooks). Rule: when a dispatched agent shows idle without a report, ping once, then switch the load to Astra web research (`scripts/ai-review.sh ... web`) and diagnose the channel separately; never wait on it.
